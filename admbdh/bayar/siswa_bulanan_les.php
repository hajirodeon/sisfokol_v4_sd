...
//nilai
$filenya = "siswa_bulanan_les.php";
$judul = "Laporan Bulanan : Uang LES";
...
echo '</select>
[<a href="siswa_bulanan_les_prt.php?tapelkd='.$tapelkd.'&utgl='.$utgl.'&ubln='.$ubln.'&uthn='.$uthn.'"><img src="'.$sumber.'/img/print.gif" border="0" width="16" height="16"></a>]
</td>
</tr>
</table>';
...
for ($i=1;$i<=$tkhir;$i++)
	...
	//query bayarnya...
	$qcc1 = mysql_query("SELECT DISTINCT(siswa_uang_les.kd_siswa) AS kd_siswa, ".
							"m_siswa.* ".
							...
							"AND round(DATE_FORMAT(siswa_uang_les.tgl_bayar, '%d')) = '$i' ".
							"AND round(DATE_FORMAT(siswa_uang_les.tgl_bayar, '%m')) = '$ubln' ".
							"AND round(DATE_FORMAT(siswa_uang_les.tgl_bayar, '%Y')) = '$uthn' ".
...
//ketahui jumlah uang les-nya... sebulan
