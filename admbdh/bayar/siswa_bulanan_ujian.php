...
//nilai
$filenya = "siswa_bulanan_ujian.php";
$judul = "Laporan Bulanan : Uang Ujian";
...
echo '</select>
[<a href="siswa_bulanan_ujian_prt.php?tapelkd='.$tapelkd.'&utgl='.$utgl.'&ubln='.$ubln.'&uthn='.$uthn.'"><img src="'.$sumber.'/img/print.gif" border="0" width="16" height="16"></a>]
</td>
</tr>
</table>';
...
for ($i=1;$i<=$tkhir;$i++)
	...
	$qcc1 = mysql_query("SELECT m_siswa.*, ".
							"siswa_uang_ujian.kd AS pkd, ".
							"siswa_uang_ujian.kd_siswa AS kd_siswa ".
							"FROM siswa_uang_ujian, m_siswa ".
							"WHERE siswa_uang_ujian.kd_siswa = m_siswa.kd ".
							"AND siswa_uang_ujian.kd_tapel = '$tapelkd' ".
							"AND siswa_uang_ujian.nilai <> '' ".
							"AND round(DATE_FORMAT(siswa_uang_ujian.tgl_bayar, '%d')) = '$i' ".
							"AND round(DATE_FORMAT(siswa_uang_ujian.tgl_bayar, '%m')) = '$ubln' ".
							"AND round(DATE_FORMAT(siswa_uang_ujian.tgl_bayar, '%Y')) = '$uthn' ".
	...
	do
		...
		//jumlah bayar ujian
		$qjmx = mysql_query("SELECT * FROM siswa_uang_ujian ".
	...
	//ketahui jumlah uang ujian-nya...
	$qjmx1 = mysql_query("SELECT SUM(nilai) AS total ".
							"FROM siswa_uang_ujian ".
							"WHERE kd_tapel = '$tapelkd' ".
							"AND nilai <> '' ".
							"AND round(DATE_FORMAT(tgl_bayar, '%d')) = '$i' ".
							"AND round(DATE_FORMAT(tgl_bayar, '%m')) = '$ubln' ".
...
//ketahui jumlah uang ujian-nya... sebulan
