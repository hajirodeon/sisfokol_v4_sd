...
//nilai
$filenya = "siswa_ulang.php";
$judul = "Keuangan Siswa : Uang Daftar Ulang";
...
//query
mysql_query("DELETE FROM siswa_uang_ulang ".
...
//yang telah dibayar
$qcc = mysql_query("SELECT SUM(nilai) AS nilai FROM siswa_uang_ulang ".
						"WHERE kd_siswa = '$swkd'");
...
if (($cc_nilai + $nil_bayar) > $pkl_nilai)
	{
	//re-direct
	$pesan = "Jumlah Uang Daftar Ulang Yang Dibayarkan, Melebihi Total Sisa Yang Ada. Harap Diperhatikan...!!";
	$ke = "$filenya?tapelkd=$tapelkd&nis=$nis";
	pekem($pesan,$ke);
	exit();
	}
	...
	{
	mysql_query("INSERT INTO siswa_uang_ulang (kd, kd_tapel, kd_siswa, tgl_bayar, nilai, postdate) VALUES ".
					"('$x', '$tapelkd', '$swkd', '$today', '$nil_bayar', '$today3')");
	}


//re-direct
$ke = "siswa_ulang_prt.php?tapelkd=$tapelkd&nis=$nis";
xloc($ke);
exit();
}
}
...
//yang telah dibayar
$qccx = mysql_query("SELECT SUM(nilai) AS nilai FROM siswa_uang_ulang ".
...
//daftar
$qdftx = mysql_query("SELECT siswa_uang_ulang.*, ".
