...
//nilai
$filenya = "siswa_tgl_les.php";
$judul = "Laporan per Tanggal : Uang Les";
...
//query
$qcc = mysql_query("SELECT DISTINCT(siswa_uang_les.tgl_bayar) AS tglku ".
			"FROM siswa_uang_les, m_siswa ".
			"WHERE siswa_uang_les.kd_siswa = m_siswa.kd ".
			"AND siswa_uang_les.nilai <> '' ".
			"AND (siswa_uang_les.tgl_bayar >= '$tgl_awal' ".
			"AND siswa_uang_les.tgl_bayar <= '$tgl_akhir') ".
			"ORDER BY siswa_uang_les.tgl_bayar ASC");
...
if ($tcc != 0)
	{
	echo '<p>[<a href="siswa_tgl_les_prt.php?tapelkd='.$tapelkd.'&utgl='.$utgl.'&ubln='.$ubln.'&uthn='.$uthn.'&utgl2='.$utgl2.'&ubln2='.$ubln2.'&uthn2='.$uthn2.'"><img src="'.$sumber.'/img/print.gif" border="0" width="16" height="16"></a>]</p>';
	...
	do
		...
		//query
		$qcc1 = mysql_query("SELECT DISTINCT(siswa_uang_les.kd_siswa) AS swkd ".
					"FROM siswa_uang_les, m_siswa ".
					"WHERE siswa_uang_les.kd_siswa = m_siswa.kd ".
					"AND siswa_uang_les.kd_tapel = '$tapelkd' ".
					"AND siswa_uang_les.nilai <> '' ".
					"AND siswa_uang_les.tgl_bayar = '$cc_tgl' ".
		...
		do
			...
			//jumlah bayar les
			$qjmx = mysql_query("SELECT * FROM siswa_uang_les ".
			...
			$jmx_nilai = nosql($rjmx['nilku']);


			//ketahui jumlah uang les-nya...
			...
			//ruang kelas
			$qnily = mysql_query("SELECT m_uang_les.*, siswa_kelas.* ".
