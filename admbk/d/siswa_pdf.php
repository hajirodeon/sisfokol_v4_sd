...
$y_kd_kecamatan = nosql($rnil['kd_kecamatan']);
$y_telp = balikin2($rnil['telp']);
$y_tinggal_dgn = balikin2($rnil['tinggal_dgn']);
$y_jarak = balikin2($rnil['jarak']);

//detail kecamatan
$qkec = mysql_query("SELECT * FROM m_kecamatan ".
			"WHERE kd = '$y_kd_kecamatan'");
$rkec = mysql_fetch_assoc($qkec);
$tkec = mysql_num_rows($qkec);
$kec_kecamatan = balikin($rkec['kecamatan']);
...
$qnil = mysql_query("SELECT m_siswa_pendidikan.*, ".
			...
			"AND siswa_kelas.kd_siswa = '$swkd'");
$rnil = mysql_fetch_assoc($qnil);
$y_kd_asal_sekolah = nosql($rnil['kd_asal_sekolah']);
$y_nama_sekolah = balikin($rnil['nama_sekolah']);
...
$y_uasbn = nosql($rnil['r_uasbn']);


//asal sekolah
$qsko = mysql_query("SELECT * FROM m_asal_sekolah ".
			"WHERE kd = '$y_kd_asal_sekolah'");
$rsko = mysql_fetch_assoc($qsko);
$y_asal_sekolah = balikin($rsko['sekolah']);
...
<tr valign="top">
<td width="200">
     <dd>a. Asal Sekolah</dd>
</td>
<td width="10">: </td>
<td>
'.$y_asal_sekolah.'.
Nama Sekolah : '.$y_nama_sekolah.'
</td>
</tr>
...
$y_agmkd = nosql($rnil['kd_agama']);
$y_warga_negara = balikin2($rnil['warga_negara']);
$y_kd_pendidikan = nosql($rnil['kd_pendidikan']);
$y_nama_sekolah = balikin($rnil['nama_sekolah']);
...
$qpki = mysql_query("SELECT * FROM m_pendidikan ".
			"WHERE kd = '$y_kd_pendidikan'");
$rpki = mysql_fetch_assoc($qpki);
$y_pendidikan = balikin($rpki['pendidikan']);
...
$qnil = mysql_query("SELECT m_siswa_selesai.*, ".
			...
			"AND siswa_kelas.kd_siswa = '$swkd'");
$rnil = mysql_fetch_assoc($qnil);
$y_kd_melanjutkan = nosql($rnil['kd_melanjutkan']);
...
//terpilih
$qljk = mysql_query("SELECT * FROM m_melanjutkan ".
			"WHERE kd = '$y_kd_melanjutkan'");
$rljk = mysql_fetch_assoc($qljk);
$y_melanjutkan = balikin($rljk['melanjutkan']);
