...
<option value="'.$filenya.'?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&rukd='.$rukd.'&jnskd=spi&jenis=SPI">SPI</option>
<option value="'.$filenya.'?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&rukd='.$rukd.'&jnskd=komite&jenis=Komite">Komite</option>
<option value="'.$filenya.'?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&rukd='.$rukd.'&jnskd=les&jenis=LES">LES</option>';
...
if ($tdt != 0)
	{
		...
		echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&rukd='.$rukd.'&jnskd='.$dt_kd.'&jenis='.$dt_nama.'">'.$dt_nama.'</option>';
		}
...
if (empty($jnskd))
	...
	//jika les ////////////////////////////////////////////////////////////////////////////////////////
		...
		$qccx = mysql_query("SELECT SUM(siswa_uang_les.nilai) AS total ".
					...
					"AND siswa_uang_les.kd_siswa = '$kd2_session'");
		$rccx = mysql_fetch_assoc($qccx);
		...
		$qdftx2 = mysql_query("SELECT SUM(siswa_uang_les.nilai) AS total ".
					"FROM siswa_uang_les ".
					"WHERE siswa_uang_les.kd_siswa = '$kd2_session' ".
