...
if ($_POST['btnEX'])
	...
	$worksheet1->write_string(0,28,"PEGAWAI_NUPTK");
	$worksheet1->write_string(0,29,"PEGAWAI_KARPEG");
	$worksheet1->write_string(0,30,"KERJA_PANGKAT");
	$worksheet1->write_string(0,31,"KERJA_JABATAN");
	$worksheet1->write_string(0,32,"KERJA_TMT_AWAL");
	$worksheet1->write_string(0,33,"KERJA_TMT_AKHIR");
	...
	do
		...
		$worksheet1->write_string($dt_nox,28,$cc_pegawai_nuptk);
		$worksheet1->write_string($dt_nox,29,$cc_pegawai_karpeg);
		$worksheet1->write_string($dt_nox,30,$cc_kerja_pangkat);
		$worksheet1->write_string($dt_nox,31,$cc_kerja_jabatan);
		$worksheet1->write_string($dt_nox,32,$cc_kerja_tmt);
		$worksheet1->write_string($dt_nox,33,$cc_kerja_tmt2);
		}
		...
		if ((empty($nip)) OR (empty($nama1)))
			...
			if ($tcc != 0)
				...
				mysql_query("INSERT INTO m_user(kd, usernamex, passwordx, nomor, nama, tipe, postdate) VALUES ".
						"('$kd', '$x_userx', '$x_passx', '$nip', '$nama1', 'GURU', '$today')");
	...
	if (empty($filex_namex))
		...
		if ($ext_filex == ".jpg")
			...
			if (!file_exists($path1))
				...
				//chmod
	                        $path3 = "../../filebox/pegawai/$kd/$filex_namex";
				chmod($path1,0755);
				chmod($path3,0755);
