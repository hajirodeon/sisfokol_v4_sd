...
if ($_POST['btnSMP2'])
	...
	if (empty($ekstra))
		...
		$ke = "$filenya?swkd=$swkd&skkd=$skkd&tapelkd=$tapelkd&kelkd=$kelkd&rukd=$rukd&smtkd=$smtkd";
		pekem($pesan,$ke);
		exit();
		}
	else
...
if ($_POST['btnSMP6'])
	...
	if (empty($tpfkd))
		...
		if ($tcc != 0)
			...
			//re-direct
			$pesan = "Kenaikan atau Tinggal Kelas, Berhasil Dilakukan. Silahkan Lakukan Penempatan Ruang Kelas untuk Siswa Tersebut...!!";
			$ke = "$filenya?swkd=$swkd&skkd=$skkd&tapelkd=$tapelkd&kelkd=$kelkd&rukd=$rukd&smtkd=$smtkd";
			pekem($pesan,$ke);
			exit();
			}
...
xheadline($judul);
echo '</td>
<td>[<a href="detail.php?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&rukd='.$rukd.'" title="Daftar Siswa">Daftar Siswa</a>]</td>
</table>
